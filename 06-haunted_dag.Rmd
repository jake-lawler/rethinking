...
data_pub <- bind_cols(Trustworthiness = tw, Newsworthiness= nw,Selected =selected) %>%
  mutate(Selected = if_else(Selected=="TRUE","Yes","No"))

ggplot(mapping=aes(Newsworthiness,Trustworthiness,colour=Selected))+
  geom_point(data=data_pub)+
    geom_smooth(data=filter(data_pub,Selected=="Yes"),method=lm,se=FALSE)
...
data_leg <- bind_cols(height,leg_left,leg_right)
...
ggplot(data=precis(m6.1))+
  geom_pointrange(aes(x=rownames(precis(m6.1)),y=mean,ymin=`5.5%`,ymax=`94.5%`))+
...
data_leg_post <- extract.samples(m6.1) %>%
  mutate(`sum of bl and br`=br+bl)

ggplot(data_leg_post)+
  geom_point(aes(x=br,y=bl),col="light blue",alpha=0.2)

ggplot(data_leg_post)+
  geom_density(aes(x=`sum of bl and br`),col="blue")
...
ggplot(data=precis( m6.7 , depth=2 ))+
  geom_pointrange(aes(x=rownames(precis( m6.7 , depth=2 )),y=mean,ymin=`5.5%`,ymax=`94.5%`))+
  geom_hline(yintercept = 0,col="red")+
  xlab("parameter")+
  coord_flip()+
  ggtitle("Including Fungus as Variable")

ggplot(data=precis( m6.8 , depth=2 ))+
  geom_pointrange(aes(x=rownames(precis( m6.8 , depth=2 )),y=mean,ymin=`5.5%`,ymax=`94.5%`))+
  geom_hline(yintercept = 0,col="red")+
  xlab("parameter")+
  coord_flip()+
